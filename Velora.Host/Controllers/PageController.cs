...
namespace Velora.Host.Controllers
{
    ...
    public class PageController : ControllerBase, IDynamicFormController<PageCrud>
    {
        private readonly IPageService _PageService;
        private readonly ITransactionService _transactionService;

        public PageController(IPageService PageService, ITransactionService transactionService)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] PageCrud Page)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageDto>
                {
                    Success = false,
            ...
            var result = await _PageService.CreateAsync(Page);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] PageCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageDto>
                {
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _PageService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _PageService.ExportAsync(
