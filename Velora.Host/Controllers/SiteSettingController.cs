...
namespace Velora.Host.Controllers
{
    ...
    public class SiteSettingController : ControllerBase, IDynamicFormController<SiteSettingCrud>
    {
        private readonly ISiteSettingService _SiteSettingService;
        private readonly ITransactionService _transactionService;

        public SiteSettingController(ISiteSettingService SiteSettingService, ITransactionService transactionService)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SiteSettingCrud SiteSetting)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SiteSettingDto>
                {
                    Success = false,
            ...
            var result = await _SiteSettingService.CreateAsync(SiteSetting);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SiteSettingCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SiteSettingDto>
                {
            ...
            var result = await _SiteSettingService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SiteSettingService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SiteSettingService.ExportAsync(
