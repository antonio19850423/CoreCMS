...
namespace Velora.Host.Controllers
{
    ...
    public class ProductVariantController : ControllerBase, IDynamicFormController<ProductVariantCrud>
    {
        private readonly IProductVariantService _ProductVariantService;
        private readonly ITransactionService _transactionService;

        public ProductVariantController(IProductVariantService ProductVariantService, ITransactionService transactionService)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ProductVariantCrud ProductVariant)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductVariantDto>
                {
                    Success = false,
            ...
            var result = await _ProductVariantService.CreateAsync(ProductVariant);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ProductVariantCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductVariantDto>
                {
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ProductVariantService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ProductVariantService.ExportAsync(
