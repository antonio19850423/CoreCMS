...
namespace Velora.Host.Controllers
{
    ...
    public class DiscountItemController : ControllerBase, IDynamicFormController<DiscountItemCrud>
    {
        private readonly IDiscountItemService _DiscountItemService;
        private readonly ITransactionService _transactionService;

        public DiscountItemController(IDiscountItemService DiscountItemService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _DiscountItemService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _DiscountItemService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] DiscountItemCrud DiscountItem)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<DiscountItemDto>
                {
                    Success = false,
            ...
            var result = await _DiscountItemService.CreateAsync(DiscountItem);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] DiscountItemCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<DiscountItemDto>
                {
            ...
            var result = await _DiscountItemService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _DiscountItemService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _DiscountItemService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _DiscountItemService.ExportAsync(
