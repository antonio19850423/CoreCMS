...
namespace Velora.Host.Controllers
{
    ...
    public class ShippingMethodCityController : ControllerBase, IDynamicFormController<ShippingMethodCityCrud>
    {
        private readonly IShippingMethodCityService _ShippingMethodCityService;
        private readonly ITransactionService _transactionService;

        public ShippingMethodCityController(IShippingMethodCityService ShippingMethodCityService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ShippingMethodCityService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ShippingMethodCityService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ShippingMethodCityCrud ShippingMethodCity)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ShippingMethodCityDto>
                {
                    Success = false,
            ...
            var result = await _ShippingMethodCityService.CreateAsync(ShippingMethodCity);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ShippingMethodCityCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ShippingMethodCityDto>
                {
            ...
            var result = await _ShippingMethodCityService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ShippingMethodCityService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ShippingMethodCityService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ShippingMethodCityService.ExportAsync(
