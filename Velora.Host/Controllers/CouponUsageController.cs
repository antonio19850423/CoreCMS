...
namespace Velora.Host.Controllers
{
    ...
    public class CouponUsageController : ControllerBase, IDynamicFormController<CouponUsageCrud>
    {
        private readonly ICouponUsageService _CouponUsageService;
        private readonly ITransactionService _transactionService;

        public CouponUsageController(ICouponUsageService CouponUsageService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _CouponUsageService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _CouponUsageService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] CouponUsageCrud CouponUsage)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<CouponUsageDto>
                {
                    Success = false,
            ...
            var result = await _CouponUsageService.CreateAsync(CouponUsage);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] CouponUsageCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<CouponUsageDto>
                {
            ...
            var result = await _CouponUsageService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _CouponUsageService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _CouponUsageService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _CouponUsageService.ExportAsync(
