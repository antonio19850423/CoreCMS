...
namespace Velora.Host.Controllers
{
    ...
    public class CategoryAttributeController : ControllerBase, IDynamicFormController<CategoryAttributeCrud>
    {
        private readonly ICategoryAttributeService _CategoryAttributeService;
        private readonly ITransactionService _transactionService;

        public CategoryAttributeController(ICategoryAttributeService CategoryAttributeService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _CategoryAttributeService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _CategoryAttributeService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] CategoryAttributeCrud CategoryAttribute)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<CategoryAttributeDto>
                {
                    Success = false,
            ...
            var result = await _CategoryAttributeService.CreateAsync(CategoryAttribute);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] CategoryAttributeCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<CategoryAttributeDto>
                {
            ...
            var result = await _CategoryAttributeService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _CategoryAttributeService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _CategoryAttributeService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _CategoryAttributeService.ExportAsync(
