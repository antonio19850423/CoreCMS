...
namespace Velora.Host.Controllers
{
    ...
    public class SmsSettingController : ControllerBase, IDynamicFormController<SmsSettingCrud>
    {
        private readonly ISmsSettingService _SmsSettingService;
        private readonly ITransactionService _transactionService;

        public SmsSettingController(ISmsSettingService SmsSettingService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _SmsSettingService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _SmsSettingService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SmsSettingCrud SmsSetting)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SmsSettingDto>
                {
                    Success = false,
            ...
            var result = await _SmsSettingService.CreateAsync(SmsSetting);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SmsSettingCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SmsSettingDto>
                {
            ...
            var result = await _SmsSettingService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _SmsSettingService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SmsSettingService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SmsSettingService.ExportAsync(
