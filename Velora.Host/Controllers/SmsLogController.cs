...
namespace Velora.Host.Controllers
{
    ...
    public class SmsLogController : ControllerBase, IDynamicFormController<SmsLogCrud>
    {
        private readonly ISmsLogService _SmsLogService;
        private readonly ITransactionService _transactionService;

        public SmsLogController(ISmsLogService SmsLogService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _SmsLogService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _SmsLogService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SmsLogCrud SmsLog)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SmsLogDto>
                {
                    Success = false,
            ...
            var result = await _SmsLogService.CreateAsync(SmsLog);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SmsLogCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SmsLogDto>
                {
            ...
            var result = await _SmsLogService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _SmsLogService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SmsLogService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SmsLogService.ExportAsync(
