...
namespace Velora.Host.Controllers
{
    ...
    public class ProductBrandController : ControllerBase, IDynamicFormController<ProductBrandCrud>
    {
        private readonly IProductBrandService _ProductBrandService;
        private readonly ITransactionService _transactionService;

        public ProductBrandController(IProductBrandService ProductBrandService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ProductBrandService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ProductBrandService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ProductBrandCrud ProductBrand)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductBrandDto>
                {
                    Success = false,
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ProductBrandCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductBrandDto>
                {
            ...
            var result = await _ProductBrandService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ProductBrandService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ProductBrandService.ExportAsync(
