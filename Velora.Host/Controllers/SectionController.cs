...
namespace Velora.Host.Controllers
{
    ...
    public class SectionController : ControllerBase, IDynamicFormController<SectionCrud>
    {
        private readonly ISectionService _SectionService;
        private readonly ITransactionService _transactionService;

        public SectionController(ISectionService SectionService, ITransactionService transactionService)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SectionCrud Section)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionDto>
                {
                    Success = false,
            ...
            var result = await _SectionService.CreateAsync(Section);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SectionCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionDto>
                {
            ...
            var result = await _SectionService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SectionService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SectionService.ExportAsync(
