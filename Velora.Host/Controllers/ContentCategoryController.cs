...
namespace Velora.Host.Controllers
{
    ...
    public class ContentCategoryController : ControllerBase, IDynamicFormController<ContentCategoryCrud>
    {
        private readonly IContentCategoryService _ContentCategoryService;
        private readonly ITransactionService _transactionService;

        public ContentCategoryController(IContentCategoryService ContentCategoryService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ContentCategoryService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ContentCategoryService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ContentCategoryCrud ContentCategory)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ContentCategoryDto>
                {
                    Success = false,
            ...
            var result = await _ContentCategoryService.CreateAsync(ContentCategory);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ContentCategoryCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ContentCategoryDto>
                {
            ...
            var result = await _ContentCategoryService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ContentCategoryService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ContentCategoryService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ContentCategoryService.ExportAsync(
