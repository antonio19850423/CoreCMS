...
namespace Velora.Host.Controllers
{
    ...
    public class ShoppingCartItemController : ControllerBase, IDynamicFormController<ShoppingCartItemCrud>
    {
        private readonly IShoppingCartItemService _ShoppingCartItemService;
        private readonly ITransactionService _transactionService;

        public ShoppingCartItemController(IShoppingCartItemService ShoppingCartItemService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ShoppingCartItemService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ShoppingCartItemService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ShoppingCartItemCrud ShoppingCartItem)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ShoppingCartItemDto>
                {
                    Success = false,
            ...
            var result = await _ShoppingCartItemService.CreateAsync(ShoppingCartItem);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ShoppingCartItemCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ShoppingCartItemDto>
                {
            ...
            var result = await _ShoppingCartItemService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ShoppingCartItemService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ShoppingCartItemService.ExportAsync(
