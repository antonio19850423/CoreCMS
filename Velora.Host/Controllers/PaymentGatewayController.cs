...
namespace Velora.Host.Controllers
{
    ...
    public class PaymentGatewayController : ControllerBase, IDynamicFormController<PaymentGatewayCrud>
    {
        private readonly IPaymentGatewayService _PaymentGatewayService;
        private readonly ITransactionService _transactionService;

        public PaymentGatewayController(IPaymentGatewayService PaymentGatewayService, ITransactionService transactionService)
        {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] PaymentGatewayCrud PaymentGateway)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PaymentGatewayDto>
                {
                    Success = false,
            ...
            var result = await _PaymentGatewayService.CreateAsync(PaymentGateway);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] PaymentGatewayCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PaymentGatewayDto>
                {
            ...
            var result = await _PaymentGatewayService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _PaymentGatewayService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _PaymentGatewayService.ExportAsync(
