...
namespace Velora.Host.Controllers
{
    ...
    public class PaymentStatusLogController : ControllerBase, IDynamicFormController<PaymentStatusLogCrud>
    {
        private readonly IPaymentStatusLogService _PaymentStatusLogService;
        private readonly ITransactionService _transactionService;

        public PaymentStatusLogController(IPaymentStatusLogService PaymentStatusLogService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _PaymentStatusLogService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _PaymentStatusLogService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] PaymentStatusLogCrud PaymentStatusLog)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PaymentStatusLogDto>
                {
                    Success = false,
            ...
            var result = await _PaymentStatusLogService.CreateAsync(PaymentStatusLog);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] PaymentStatusLogCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PaymentStatusLogDto>
                {
            ...
            var result = await _PaymentStatusLogService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _PaymentStatusLogService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _PaymentStatusLogService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _PaymentStatusLogService.ExportAsync(
