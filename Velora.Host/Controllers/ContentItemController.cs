...
namespace Velora.Host.Controllers
{
    ...
    public class ContentItemController : ControllerBase, IDynamicFormController<ContentItemCrud>
    {
        private readonly IContentItemService _ContentItemService;
        private readonly ITransactionService _transactionService;

        public ContentItemController(IContentItemService ContentItemService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ContentItemService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ContentItemService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ContentItemCrud ContentItem)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ContentItemDto>
                {
                    Success = false,
            ...
            var result = await _ContentItemService.CreateAsync(ContentItem);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ContentItemCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ContentItemDto>
                {
            ...
            var result = await _ContentItemService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ContentItemService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ContentItemService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ContentItemService.ExportAsync(
