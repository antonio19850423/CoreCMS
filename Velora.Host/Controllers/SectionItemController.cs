...
namespace Velora.Host.Controllers
{
    ...
    public class SectionItemController : ControllerBase, IDynamicFormController<SectionItemCrud>
    {
        private readonly ISectionItemService _SectionItemService;
        private readonly ITransactionService _transactionService;

        public SectionItemController(ISectionItemService SectionItemService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _SectionItemService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _SectionItemService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SectionItemCrud SectionItem)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionItemDto>
                {
                    Success = false,
            ...
            var result = await _SectionItemService.CreateAsync(SectionItem);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SectionItemCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionItemDto>
                {
            ...
            var result = await _SectionItemService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _SectionItemService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SectionItemService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SectionItemService.ExportAsync(
