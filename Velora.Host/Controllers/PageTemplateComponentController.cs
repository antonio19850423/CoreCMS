...
namespace Velora.Host.Controllers
{
    ...
    public class PageTemplateComponentController : ControllerBase, IDynamicFormController<PageTemplateComponentCrud>
    {
        private readonly IPageTemplateComponentService _PageTemplateComponentService;
        private readonly ITransactionService _transactionService;

        public PageTemplateComponentController(IPageTemplateComponentService PageTemplateComponentService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _PageTemplateComponentService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _PageTemplateComponentService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] PageTemplateComponentCrud PageTemplateComponent)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageTemplateComponentDto>
                {
                    Success = false,
            ...
            var result = await _PageTemplateComponentService.CreateAsync(PageTemplateComponent);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] PageTemplateComponentCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageTemplateComponentDto>
                {
            ...
            var result = await _PageTemplateComponentService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _PageTemplateComponentService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _PageTemplateComponentService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _PageTemplateComponentService.ExportAsync(
