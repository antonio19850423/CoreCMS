...
namespace Velora.Host.Controllers
{
    ...
    public class ProductTagController : ControllerBase, IDynamicFormController<ProductTagCrud>
    {
        private readonly IProductTagService _ProductTagService;
        private readonly ITransactionService _transactionService;

        public ProductTagController(IProductTagService ProductTagService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ProductTagService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ProductTagService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ProductTagCrud ProductTag)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductTagDto>
                {
                    Success = false,
            ...
            var result = await _ProductTagService.CreateAsync(ProductTag);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ProductTagCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductTagDto>
                {
            ...
            var result = await _ProductTagService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ProductTagService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ProductTagService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ProductTagService.ExportAsync(
