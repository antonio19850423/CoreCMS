...
namespace Velora.Host.Controllers
{
    ...
    public class PageTemplateController : ControllerBase, IDynamicFormController<PageTemplateCrud>
    {
        private readonly IPageTemplateService _PageTemplateService;
        private readonly ITransactionService _transactionService;

        public PageTemplateController(IPageTemplateService PageTemplateService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _PageTemplateService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _PageTemplateService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] PageTemplateCrud PageTemplate)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageTemplateDto>
                {
                    Success = false,
            ...
            var result = await _PageTemplateService.CreateAsync(PageTemplate);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] PageTemplateCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<PageTemplateDto>
                {
            ...
            var result = await _PageTemplateService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _PageTemplateService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _PageTemplateService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _PageTemplateService.ExportAsync(
