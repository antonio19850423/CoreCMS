...
namespace Velora.Host.Controllers
{
    ...
    public class SiteMenuController : ControllerBase, IDynamicFormController<SiteMenuCrud>
    {
        private readonly ISiteMenuService _SiteMenuService;
        private readonly ITransactionService _transactionService;

        public SiteMenuController(ISiteMenuService SiteMenuService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _SiteMenuService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _SiteMenuService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SiteMenuCrud SiteMenu)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SiteMenuDto>
                {
                    Success = false,
            ...
            var result = await _SiteMenuService.CreateAsync(SiteMenu);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SiteMenuCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SiteMenuDto>
                {
            ...
            var result = await _SiteMenuService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _SiteMenuService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SiteMenuService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SiteMenuService.ExportAsync(
