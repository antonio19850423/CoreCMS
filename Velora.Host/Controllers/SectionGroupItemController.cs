...
namespace Velora.Host.Controllers
{
    ...
    public class SectionGroupItemController : ControllerBase, IDynamicFormController<SectionGroupItemCrud>
    {
        private readonly ISectionGroupItemService _SectionGroupItemService;
        private readonly ITransactionService _transactionService;

        public SectionGroupItemController(ISectionGroupItemService SectionGroupItemService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _SectionGroupItemService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _SectionGroupItemService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] SectionGroupItemCrud SectionGroupItem)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionGroupItemDto>
                {
                    Success = false,
            ...
            var result = await _SectionGroupItemService.CreateAsync(SectionGroupItem);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] SectionGroupItemCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<SectionGroupItemDto>
                {
            ...
            var result = await _SectionGroupItemService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _SectionGroupItemService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _SectionGroupItemService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _SectionGroupItemService.ExportAsync(
