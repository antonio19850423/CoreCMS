...
namespace Velora.Host.Controllers
{
    ...
    public class ProductFileController : ControllerBase, IDynamicFormController<ProductFileCrud>
    {
        private readonly IProductFileService _ProductFileService;
        private readonly ITransactionService _transactionService;

        public ProductFileController(IProductFileService ProductFileService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ProductFileService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ProductFileService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ProductFileCrud ProductFile)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductFileDto>
                {
                    Success = false,
            ...
            var result = await _ProductFileService.CreateAsync(ProductFile);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ProductFileCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductFileDto>
                {
            ...
            var result = await _ProductFileService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ProductFileService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ProductFileService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ProductFileService.ExportAsync(
