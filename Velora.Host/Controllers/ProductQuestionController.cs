...
namespace Velora.Host.Controllers
{
    ...
    public class ProductQuestionController : ControllerBase, IDynamicFormController<ProductQuestionCrud>
    {
        private readonly IProductQuestionService _ProductQuestionService;
        private readonly ITransactionService _transactionService;

        public ProductQuestionController(IProductQuestionService ProductQuestionService, ITransactionService transactionService)
        {
        ...
        {
            var result = await _ProductQuestionService.GetAllAsync();
            return Ok(result);
        }
        ...
        {
            var result = await _ProductQuestionService.GetByIdAsync(id);
            if (!result.Success)
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] ProductQuestionCrud ProductQuestion)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductQuestionDto>
                {
                    Success = false,
            ...
            var result = await _ProductQuestionService.CreateAsync(ProductQuestion);
            if (!result.Success)
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] ProductQuestionCrud dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(new ResultDto<ProductQuestionDto>
                {
            ...
            var result = await _ProductQuestionService.UpdateAsync(dto);
            if (!result.Success)
                return NotFound(result);
        ...
        {
            var result = await _ProductQuestionService.DeleteAsync(id);
            if (!result.Success)
                return NotFound(result);
        ...
        public async Task<IActionResult> BulkInsert()
        {
            ...
            var result = await _ProductQuestionService.BulkInsertAsync(stream);
        ...
        public async Task<IActionResult> Export([FromBody] ExportRequestDto request)
        {
            ...
            try
            {
                fileBytes = await _ProductQuestionService.ExportAsync(
