...
public class ProductFileGqlResolver : IProductFileGqlResolver
{
    IProductFileService _ProductFileService;
    public ProductFileGqlResolver(IProductFileService ProductFileService)
    {
    ...
    /// <returns></returns>
    [Authorize]
    [GraphQLName("productFileView")]
    [UsePaging(IncludeTotalCount = true)]
    [UseFiltering]
    [UseSorting]
    public async Task<IQueryable<ProductFileCrud>> productFileView()
    {
        var query = await _ProductFileService
            .GetAllViewQueryable<SqlProductFileView, SqlProductFileView, ProductFileCrud>();
        return query.Select(x => new ProductFileCrud
        {
            Id = x.Id,
            ParentId = x.ParentId,
            IsActive = x.IsActive,
