...
public class ProductTagGqlResolver : IProductTagGqlResolver
{
    IProductTagService _ProductTagService;
    public ProductTagGqlResolver(IProductTagService ProductTagService)
    {
    ...
    /// <returns></returns>
    [Authorize]
    [GraphQLName("productTagView")]
    [UsePaging(IncludeTotalCount = true)]
    [UseFiltering]
    [UseSorting]
    public async Task<IQueryable<ProductTagCrud>> productTagView()
    {
        var query = await _ProductTagService
            .GetAllViewQueryable<SqlProductTagView, SqlProductTagView, ProductTagCrud>();
        return query.Select(x => new ProductTagCrud
        {
            Id = x.Id,
            Color = x.Color??"",
            Slug=x.Slug??"",
